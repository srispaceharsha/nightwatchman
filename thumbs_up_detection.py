...
def main():
    ...
    print("[INFO] Starting thumbs-up detector...")
    print(f"[INFO] Need to hold thumbs up for {HOLD_SECONDS} seconds.")
        ...
        while cap.isOpened():
            ...
            if results.multi_hand_landmarks:
                for hand_landmarks in results.multi_hand_landmarks:
                    ...
                    is_up, variant, _ = classify_thumbs_up(hand_landmarks)
                    if is_up:
                        thumbs_up_now = True
            ...
            if thumbs_up_now:
                ...
                if not gesture_confirmed and elapsed >= HOLD_SECONDS:
                    ...
                    print(
                        f"{RED}👍 [CONFIRM] Thumbs up ({v}) confirmed at t={now:.3f}, "
            ...
            # UI overlay
            if thumbs_up_now and gesture_start_time is not None:
                label = f"Thumbs up ({current_variant or '...'}): {elapsed:.1f}s"
                cv2.putText(frame, label, (30, 50),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8,
                            (0, 255, 0), 2, cv2.LINE_AA)
            elif hand_present:
                cv2.putText(frame, "Hand detected, waiting for thumbs up...",
